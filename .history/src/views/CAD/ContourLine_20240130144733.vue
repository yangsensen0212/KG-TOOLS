  ...
  <div class="container">
    <el-row type="flex" justify="center" align="middle" class="row-bg">
      <el-col :span="12">
        ...
        <el-upload class="upload-cad" :on-preview="handleFilePreview" :on-remove="handleFileRemove"
          :before-upload="beforeFileUpload" :auto-upload="false" multiple>
    ...
    <el-row type="flex" justify="center">
      <div class="checkbox-container">
        <el-checkbox-group v-model="selectedLayers">
          <el-checkbox v-for="item in layerData" :label="item.name" :key="item.id">
          </el-checkbox>
        </el-checkbox-group>
      </div>
    </el-row>
    <el-row type="flex" justify="center">
      <el-col :span="12">
        ...
        <el-button type="info" size="large" icon="el-icon-document">预览表格结果</el-button>
        <el-button type="warning" size="large" icon="el-icon-data-analysis">预览曲面图结果</el-button>
    ...
    <el-row type="flex" justify="center">
      <el-col :span="10">
        <el-table :data="tableData" style="width: 100%">
          <el-table-column v-for="col in tableColumns" :key="col.prop" :prop="col.prop"
...
import data from './data/等高线.json'
export default {
  components: {
    ChartView
  },
  data() {
    return {
      ...
      selectedFile: null,
      tableData: [{ 'x': 13651116.804952, 'y': -15783369.053234, 'z': -680.0, 'line_id': 0.0 }, { 'x': 13651127.965082, 'y': -15783367.623164, 'z': -680.0, 'line_id': 0.0 }, { 'x': 13651139.127112, 'y': -15783366.208474, 'z': -680.0, 'line_id': 0.0 }, { 'x': 13651339.765086, 'y': -15779750.764515, 'z': -580.0, 'line_id': 1.0 }, { 'x': 13651337.031548, 'y': -15779747.469888, 'z': -580.0, 'line_id': 1.0 }, { 'x': 13651334.30667, 'y': -15779744.170261, 'z': -580.0, 'line_id': 1.0 }, { 'x': 13651331.607773, 'y': -15779740.855633, 'z': -580.0, 'line_id': 1.0 }, { 'x': 13651329.681095, 'y': -15779738.455589, 'z': -580.0, 'line_id': 1.0 }, { 'x': "...", "y": '...', 'z': '...', "line_id": '...' }], // 这里应放入处理后的表格数据
      tableColumns: [{
        prop: "x",
        label: "x"
      }, {
      ...
      chartOption: {
        ...
        series: [
          {
            type: 'surface',
            data: [[13643923.18992, -15781659.052855, -600.0], [13651350.71022, -15779763.902043, -580.0], [13638477.185628, -15783567.44624, -500.0], [13648693.207048, -15784578.820883, -900.0], [13642391.192564, -15786166.554133, -740.0], [13647653.423594, -15780967.868101, -440.0], [13640549.996794, -15780765.878036, -560.0], [13643352.674851, -15778502.812134, -440.0], [13645562.72785, -15784333.629574, -740.0], [13651233.051085, -15782806.082407, -720.0]],// data.data.map(item => [item[0], item[1], item[2]]),
            shading: 'lambert',
            label: {
  ...
  methods: {
    ...
    startExtraction() {
      debugger
      let a = data.data.map(item => [item[0], item[1], item[2]])
      console.log(a)
      debugger
